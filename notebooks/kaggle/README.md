...
Week 9 uses notebooks `22` through `25`. Notebook `22` freezes the fixed V/ID/OOD analysis IDs. Notebook `23` completes 72 controlled linear probes and 12 controlled k-NN probes, while reusing the verified Week 7 foundation probes. Run `24a`-`24c` sequentially on T4 x2 for spectral stress, CKA/effective-rank features, fixed retrievals, and source-distance records. Finish with CPU notebook `25`, which verifies the 108 linear probes, 18 k-NN probes, 108 stress states, and 3,000 retrieval rows. No encoder is updated and no model or threshold is selected in Week 9.
