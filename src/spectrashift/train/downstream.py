from __future__ import annotations

import json
import hashlib
import math
import os
import time
from pathlib import Path

import numpy as np
import pandas as pd
import torch
import yaml
from torch.utils.data import DataLoader

from spectrashift.data.bands import BAND_ADAPTERS
from spectrashift.data.dataset import SpectraShiftDataset
from spectrashift.data.downstream import FRACTION_COUNTS, WEEK5_SEEDS
from spectrashift.data.rgb import SpectraShiftImageNetRGBDataset
from spectrashift.data.supervised import DownstreamDataset
from spectrashift.eval.metrics import fit_global_validation_threshold, multilabel_metrics
from spectrashift.models.resnet import (
    build_imagenet_resnet18_core10,
    build_imagenet_resnet18_rgb,
    build_resnet18,
    build_resnet18_from_encoder_export,
)

from .common import (
    append_jsonl,
    atomic_torch_save,
    capture_rng_state,
    file_sha256,
    git_commit,
    hardware_record,
    object_sha256,
    restore_rng_state,
    seed_everything,
    select_device,
    source_tree_sha256,
)


MODEL_ADAPTERS = {
    "M0": "core10", "M1": "core10", "M1RGB": "rgb",
    "M2": "rgb", "M3": "core10", "M4": "core10",
}


def model_state_sha256(model: torch.nn.Module) -> str:
    digest = hashlib.sha256()
    for name, value in sorted(model.state_dict().items()):
        tensor = value.detach().cpu().contiguous()
        digest.update(name.encode())
        digest.update(str(tensor.dtype).encode())
        digest.update(str(tuple(tensor.shape)).encode())
        digest.update(tensor.numpy().tobytes())
    return digest.hexdigest()


def validate_downstream_config(config: dict[str, object]) -> None:
    run, training, data = config["run"], config["training"], config["data"]
    model_id = str(run["model_id"])
    seed = int(run["seed"])
    fraction = str(run["fraction_code"])
    if model_id not in MODEL_ADAPTERS or seed not in WEEK5_SEEDS or fraction not in FRACTION_COUNTS:
        raise ValueError("Unexpected downstream run identity")
    if int(run["sample_count"]) != FRACTION_COUNTS[fraction]:
        raise ValueError("Downstream sample count differs from the frozen fraction")
    if data["adapter"] != MODEL_ADAPTERS[model_id]:
        raise ValueError("Downstream model uses the wrong band adapter")
    preprocessing = data.get("preprocessing", {"mode": "u_standardization"})
    expected_mode = "imagenet_rgb_percentile" if model_id == "M1RGB" else "u_standardization"
    if preprocessing.get("mode") != expected_mode:
        raise ValueError(f"{model_id} requires preprocessing mode {expected_mode}")
    if model_id == "M1RGB":
        contract_path = Path(preprocessing["contract_path"])
        if file_sha256(contract_path) != preprocessing["contract_sha256"]:
            raise ValueError("M1RGB preprocessing contract hash mismatch")
    if int(training["batch_size"]) != 64:
        raise ValueError("Week 5 physical batch must be 64")
    numeric = {
        "weight_decay": 1e-4,
        "warmup_fraction": 0.05,
        "gradient_clip": 1.0,
        "amp_initial_scale": 128.0,
    }
    for key, expected in numeric.items():
        if not math.isclose(float(training[key]), expected):
            raise ValueError(f"Week 5 training.{key} must be {expected}")
    if int(training["amp_growth_interval"]) != 1_000_000:
        raise ValueError("Week 5 AMP scale growth interval must keep the verified scale fixed")
    if not training.get("amp") or not training.get("require_cuda") or not training.get("fail_on_amp_overflow"):
        raise ValueError("Week 5 requires CUDA AMP with immediate overflow failure")
    epochs, _, steps = expected_epochs_and_steps(
        int(run["sample_count"]), int(training["batch_size"]),
        int(training["base_epochs"]), int(training["minimum_optimizer_steps"]),
    )
    if steps != int(training["expected_optimizer_steps"]):
        raise ValueError("Week 5 exact step count is inconsistent with the resolved schedule")
    expected_eval = 1 if run.get("pilot") or fraction != "01" else 5
    if int(training["evaluation_every_epochs"]) != expected_eval:
        raise ValueError("Week 5 validation cadence differs from the frozen protocol")
    if epochs != int(training["base_epochs"]):
        raise ValueError("Resolved Week 5 epochs must already include the minimum-step floor")


def expected_epochs_and_steps(
    sample_count: int,
    batch_size: int,
    base_epochs: int,
    minimum_steps: int,
) -> tuple[int, int, int]:
    batches = math.ceil(sample_count / batch_size)
    if batches <= 0:
        raise ValueError("Downstream training subset is empty")
    epochs = max(base_epochs, math.ceil(minimum_steps / batches))
    return epochs, batches, epochs * batches


def _schedule(step: int, warmup_steps: int, total_steps: int, minimum_ratio: float) -> float:
    if warmup_steps and step < warmup_steps:
        return (step + 1) / warmup_steps
    progress = min(max((step - warmup_steps) / max(total_steps - warmup_steps, 1), 0.0), 1.0)
    return minimum_ratio + (1.0 - minimum_ratio) * 0.5 * (1.0 + math.cos(math.pi * progress))


def build_optimizer(
    model: torch.nn.Module,
    encoder_lr: float,
    head_lr: float,
    weight_decay: float,
) -> torch.optim.Optimizer:
    head_parameters = {id(value) for value in model.fc.parameters()}
    encoder_decay, encoder_no_decay, head_decay, head_no_decay = [], [], [], []
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad:
            continue
        target_head = id(parameter) in head_parameters
        target_decay = not (name.endswith("bias") or parameter.ndim <= 1)
        if target_head and target_decay:
            head_decay.append(parameter)
        elif target_head:
            head_no_decay.append(parameter)
        elif target_decay:
            encoder_decay.append(parameter)
        else:
            encoder_no_decay.append(parameter)
    return torch.optim.AdamW([
        {"params": encoder_decay, "lr": encoder_lr, "weight_decay": weight_decay, "group_name": "encoder_decay"},
        {"params": encoder_no_decay, "lr": encoder_lr, "weight_decay": 0.0, "group_name": "encoder_no_decay"},
        {"params": head_decay, "lr": head_lr, "weight_decay": weight_decay, "group_name": "head_decay"},
        {"params": head_no_decay, "lr": head_lr, "weight_decay": 0.0, "group_name": "head_no_decay"},
    ])


def _build_model(config: dict[str, object]) -> tuple[torch.nn.Module, dict[str, object]]:
    run = config["run"]
    model_id = str(run["model_id"])
    seed = int(run["seed"])
    initialization = config["initialization"]
    if model_id == "M0":
        model = build_resnet18(10, outputs=19)
        record = {"type": "random", "sha256": object_sha256({"model_id": model_id, "seed": seed})}
    elif model_id in {"M1", "M1RGB"}:
        path = Path(initialization["path"])
        digest = file_sha256(path)
        if not digest.startswith(str(initialization["sha256_prefix"])):
            raise ValueError("ImageNet checkpoint hash prefix does not match f37072fd")
        expected = initialization.get("sha256")
        if expected and digest != expected:
            raise ValueError("ImageNet checkpoint full hash differs from the contracts dataset")
        if model_id == "M1":
            model = build_imagenet_resnet18_core10(path, outputs=19)
            initialization_type = "imagenet1k-v1-core10"
        else:
            model = build_imagenet_resnet18_rgb(path, outputs=19)
            initialization_type = "imagenet1k-v1-rgb-unchanged-stem"
        record = {"type": initialization_type, "path": str(path), "sha256": digest}
    elif model_id in {"M2", "M3", "M4"}:
        path = Path(initialization["path"])
        digest = file_sha256(path)
        if digest != initialization["sha256"]:
            raise ValueError("Week 4 encoder hash differs from the approved aggregate")
        model, payload = build_resnet18_from_encoder_export(path, model_id, seed, outputs=19)
        record = {
            "type": "week4-encoder",
            "path": str(path),
            "sha256": digest,
            "source_checkpoint_sha256": payload["source_checkpoint_sha256"],
        }
    else:
        raise ValueError(f"Unsupported downstream model: {model_id}")
    if MODEL_ADAPTERS[model_id] != config["data"]["adapter"]:
        raise ValueError("Downstream model adapter contract is incorrect")
    record["model_state_sha256"] = model_state_sha256(model)
    return model, record


def _make_base(config: dict[str, object], partition: str):
    data = config["data"]
    preprocessing = data.get("preprocessing", {"mode": "u_standardization"})
    if preprocessing.get("mode") == "imagenet_rgb_percentile":
        return SpectraShiftImageNetRGBDataset(
            data["manifest_path"], data["staged_root"], data["normalization_path"],
            preprocessing["contract_path"], partition, int(data.get("shard_size", 512)),
            int(data.get("height", 120)), int(data.get("width", 120)),
        )
    return SpectraShiftDataset(
        data["manifest_path"], data["staged_root"], data["normalization_path"],
        partition, data["adapter"], int(data.get("shard_size", 512)),
        int(data.get("height", 120)), int(data.get("width", 120)),
    )


def _evaluate(model, dataset, device, batch_size: int, num_workers: int):
    loader = DataLoader(
        dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers,
        pin_memory=device.type == "cuda",
    )
    model.eval()
    logits, targets, patch_ids = [], [], []
    with torch.inference_mode():
        for images, labels, ids in loader:
            logits.append(model(images.to(device, non_blocking=True)).float().cpu())
            targets.append(labels)
            patch_ids.extend(map(str, ids))
    return torch.cat(logits).numpy(), torch.cat(targets).numpy(), patch_ids


def _save_predictions(path: Path, logits: np.ndarray, targets: np.ndarray, patch_ids: list[str]) -> None:
    temporary = path.with_suffix(path.suffix + ".tmp")
    with temporary.open("wb") as stream:
        np.savez_compressed(
            stream,
            logits=logits.astype(np.float32),
            targets=targets.astype(np.uint8),
            patch_ids=np.asarray(patch_ids, dtype="U"),
        )
    os.replace(temporary, path)


def verify_downstream_checkpoint(path: str | Path, expected_run_id: str | None = None) -> dict[str, object]:
    payload = torch.load(path, map_location="cpu", weights_only=False)
    required = {"run_id", "model_id", "seed", "fraction_code", "model", "config_sha256", "initialization"}
    missing = required - set(payload)
    if missing:
        raise ValueError(f"Downstream checkpoint is missing fields: {sorted(missing)}")
    if expected_run_id and payload["run_id"] != expected_run_id:
        raise ValueError("Downstream checkpoint identity mismatch")
    if any(not torch.isfinite(value).all() for value in payload["model"].values()):
        raise ValueError("Downstream checkpoint contains non-finite parameters or buffers")
    return payload


def train_downstream(
    config_path: str | Path,
    resume_path: str | Path | None = None,
) -> dict[str, object]:
    config_path = Path(config_path)
    config = yaml.safe_load(config_path.read_text())
    validate_downstream_config(config)
    run, training, data = config["run"], config["training"], config["data"]
    output_dir = Path(run["output_dir"])
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "resolved_config.yaml").write_text(yaml.safe_dump(config, sort_keys=False))
    config_hash = object_sha256(config)
    seed = int(run["seed"])
    seed_everything(seed)
    device = select_device(bool(training.get("require_cuda", False)))

    normalization = json.loads(Path(data["normalization_path"]).read_text())
    freeze = json.loads(Path(data["freeze_summary_path"]).read_text())
    if normalization.get("sha256") != data["normalization_sha256"]:
        raise ValueError("Week 2 normalization contract mismatch")
    if not freeze.get("training_approved") or freeze.get("manifest_sha256") != data["manifest_contract_sha256"]:
        raise ValueError("Week 2 manifest does not approve downstream training")
    contracts = json.loads(Path(config["subsets"]["contract_path"]).read_text())
    subset_frame = pd.read_parquet(config["subsets"]["manifest_path"])
    seed_rows = subset_frame[subset_frame["downstream_seed"].eq(seed)].sort_values("subset_rank")
    count = int(run["sample_count"])
    patch_ids = seed_rows.head(count)["patch_id"].astype(str).tolist()
    if len(patch_ids) != count or contracts["subset_manifest_sha256"] != config["subsets"]["sha256"]:
        raise ValueError("Frozen downstream subset contract mismatch")

    train_base = _make_base(config, "D")
    validation_base = _make_base(config, "V")
    train_dataset = DownstreamDataset(train_base, patch_ids, seed, training=True)
    validation_dataset = DownstreamDataset(validation_base, None, seed, training=False)
    model, initialization_record = _build_model(config)
    model = model.to(device)
    encoder_lr = float(training["encoder_learning_rate"])
    head_lr = float(training["head_learning_rate"])
    optimizer = build_optimizer(model, encoder_lr, head_lr, float(training["weight_decay"]))
    epochs, batches_per_epoch, expected_steps = expected_epochs_and_steps(
        count, int(training["batch_size"]), int(training["base_epochs"]),
        int(training["minimum_optimizer_steps"]),
    )
    if expected_steps != int(training["expected_optimizer_steps"]):
        raise ValueError("Resolved downstream run has the wrong exact step count")
    warmup_steps = max(1, int(round(expected_steps * float(training["warmup_fraction"]))))
    minimum_ratio = float(training["minimum_encoder_learning_rate"]) / encoder_lr
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lambda step: _schedule(step, warmup_steps, expected_steps, minimum_ratio)
    )
    amp_enabled = bool(training.get("amp", True)) and device.type == "cuda"
    scaler = torch.amp.GradScaler(
        "cuda", enabled=amp_enabled, init_scale=float(training["amp_initial_scale"]),
        growth_interval=int(training["amp_growth_interval"]),
    )
    criterion = torch.nn.BCEWithLogitsLoss()
    start_epoch = 0
    global_step = 0
    overflow_skips = 0
    elapsed_before_resume = 0.0
    best_map = float("-inf")
    best_epoch = -1
    best_path = output_dir / "best-model.pt"
    recovery_path = output_dir / "recovery-latest.pt"
    history: list[dict[str, object]] = []
    if resume_path:
        checkpoint = torch.load(resume_path, map_location="cpu", weights_only=False)
        if checkpoint["config_sha256"] != config_hash:
            raise ValueError("Resume checkpoint configuration differs from this run")
        model.load_state_dict(checkpoint["model"])
        optimizer.load_state_dict(checkpoint["optimizer"])
        scheduler.load_state_dict(checkpoint["scheduler"])
        scaler.load_state_dict(checkpoint["scaler"])
        restore_rng_state(checkpoint["rng_state"])
        start_epoch = int(checkpoint["next_epoch"])
        global_step = int(checkpoint["global_step"])
        overflow_skips = int(checkpoint["amp_overflow_skips"])
        elapsed_before_resume = float(checkpoint["elapsed_seconds"])
        best_map = float(checkpoint["best_map"])
        best_epoch = int(checkpoint["best_epoch"])
        history = list(checkpoint["history"])
    log_path = output_dir / "training.jsonl"
    evaluation_every = int(training["evaluation_every_epochs"])
    started = time.perf_counter()

    for epoch in range(start_epoch, epochs):
        train_dataset.set_epoch(epoch)
        generator = torch.Generator().manual_seed(seed + epoch)
        loader = DataLoader(
            train_dataset, batch_size=int(training["batch_size"]), shuffle=True,
            generator=generator, drop_last=False, num_workers=int(training["num_workers"]),
            pin_memory=device.type == "cuda", persistent_workers=False,
        )
        if len(loader) != batches_per_epoch:
            raise RuntimeError("Downstream batch count changed from the frozen schedule")
        model.train()
        losses = []
        for images, labels, _ in loader:
            optimizer.zero_grad(set_to_none=True)
            with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=amp_enabled):
                loss = criterion(model(images.to(device, non_blocking=True)), labels.to(device, non_blocking=True))
            if not torch.isfinite(loss):
                raise FloatingPointError(f"Non-finite downstream loss at step {global_step}")
            scaler.scale(loss).backward()
            scaler.unscale_(optimizer)
            gradient_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), float(training["gradient_clip"]))
            if not torch.isfinite(gradient_norm):
                overflow_skips += 1
                if training.get("fail_on_amp_overflow", True):
                    raise FloatingPointError(f"AMP overflow at downstream step {global_step}")
                continue
            if gradient_norm <= 0:
                raise FloatingPointError(f"Zero downstream gradient norm at step {global_step}")
            scaler.step(optimizer)
            scaler.update()
            scheduler.step()
            global_step += 1
            losses.append(float(loss.detach()))
        evaluate = (epoch + 1) % evaluation_every == 0 or epoch + 1 == epochs
        record: dict[str, object] = {
            "epoch": epoch + 1,
            "global_step": global_step,
            "train_loss": float(np.mean(losses)),
            "encoder_learning_rate": optimizer.param_groups[0]["lr"],
            "head_learning_rate": optimizer.param_groups[2]["lr"],
        }
        if evaluate:
            logits, targets, validation_ids = _evaluate(
                model, validation_dataset, device, int(training["validation_batch_size"]),
                int(training["num_workers"]),
            )
            probabilities = 1.0 / (1.0 + np.exp(-np.clip(logits.astype(np.float64), -80, 80)))
            metrics = multilabel_metrics(
                targets, probabilities,
                supported_indices=contracts["supported_class_indices"],
            )
            record["validation_macro_average_precision"] = metrics["macro_average_precision"]
            if float(metrics["macro_average_precision"]) > best_map:
                best_map = float(metrics["macro_average_precision"])
                best_epoch = epoch + 1
                atomic_torch_save({
                    "run_id": run["id"], "model_id": run["model_id"], "seed": seed,
                    "fraction_code": run["fraction_code"], "model": {
                        name: value.detach().cpu() for name, value in model.state_dict().items()
                    },
                    "config_sha256": config_hash, "initialization": initialization_record,
                    "epoch": best_epoch, "validation_macro_average_precision": best_map,
                }, best_path)
                _save_predictions(output_dir / "validation_predictions.npz", logits, targets, validation_ids)
        history.append(record)
        append_jsonl(log_path, record)
        if evaluate:
            atomic_torch_save({
                "config_sha256": config_hash,
                "model": {name: value.detach().cpu() for name, value in model.state_dict().items()},
                "optimizer": optimizer.state_dict(), "scheduler": scheduler.state_dict(),
                "scaler": scaler.state_dict(), "rng_state": capture_rng_state(),
                "next_epoch": epoch + 1, "global_step": global_step,
                "amp_overflow_skips": overflow_skips,
                "elapsed_seconds": elapsed_before_resume + time.perf_counter() - started,
                "best_map": best_map, "best_epoch": best_epoch, "history": history,
            }, recovery_path)

    elapsed = elapsed_before_resume + time.perf_counter() - started
    if global_step != expected_steps or overflow_skips != 0:
        raise ValueError("Downstream run did not complete its exact successful-step contract")
    best = verify_downstream_checkpoint(best_path, str(run["id"]))
    model.load_state_dict(best["model"])
    logits, targets, validation_ids = _evaluate(
        model, validation_dataset, device, int(training["validation_batch_size"]), int(training["num_workers"])
    )
    probabilities = 1.0 / (1.0 + np.exp(-np.clip(logits.astype(np.float64), -80, 80)))
    threshold = fit_global_validation_threshold(targets, probabilities)
    metrics_at_half = multilabel_metrics(
        targets, probabilities, thresholds=0.5,
        supported_indices=contracts["supported_class_indices"],
    )
    metrics_selected = multilabel_metrics(
        targets, probabilities, thresholds=float(threshold["threshold"]),
        supported_indices=contracts["supported_class_indices"],
    )
    _save_predictions(output_dir / "validation_predictions.npz", logits, targets, validation_ids)
    checkpoint_hash = file_sha256(best_path)
    predictions_hash = file_sha256(output_dir / "validation_predictions.npz")
    result = {
        "run_id": run["id"], "model_id": run["model_id"], "seed": seed,
        "fraction_code": run["fraction_code"], "sample_count": count,
        "epochs": epochs, "steps": global_step, "expected_optimizer_steps": expected_steps,
        "best_epoch": best_epoch, "validation_macro_average_precision": best_map,
        "metrics_at_0_5": metrics_at_half, "selected_global_threshold": threshold,
        "metrics_at_selected_threshold": metrics_selected,
        "amp_overflow_skips": overflow_skips, "elapsed_seconds": elapsed,
        "initialization": initialization_record,
        "adapter": data["adapter"], "band_order": list(BAND_ADAPTERS[data["adapter"]]),
        "preprocessing": data.get("preprocessing", {"mode": "u_standardization"}),
        "config_sha256": config_hash, "source_tree_sha256": source_tree_sha256(),
        "git_commit": git_commit(), "hardware": hardware_record(device),
        "data": {
            "manifest_file_sha256": file_sha256(data["manifest_path"]),
            "manifest_contract_sha256": data["manifest_contract_sha256"],
            "normalization_sha256": data["normalization_sha256"],
            "subset_manifest_sha256": contracts["subset_manifest_sha256"],
            "train_patches": count, "validation_patches": len(validation_dataset),
        },
        "best_checkpoint": str(best_path), "best_checkpoint_sha256": checkpoint_hash,
        "validation_predictions": str(output_dir / "validation_predictions.npz"),
        "validation_predictions_sha256": predictions_hash,
        "finite_gate": True, "completion_gate": True,
        "evaluation_labels_loaded": False,
    }
    (output_dir / "downstream_summary.json").write_text(json.dumps(result, indent=2) + "\n")
    recovery_path.unlink(missing_ok=True)
    return result


def cache_prefinetune_features(config_path: str | Path, output_path: str | Path) -> dict[str, object]:
    config = yaml.safe_load(Path(config_path).read_text())
    if config["run"]["model_id"] == "M0":
        raise ValueError("M0 random features are not part of the frozen probe registry")
    seed_everything(int(config["run"]["seed"]))
    device = select_device(bool(config["training"].get("require_cuda", False)))
    model, initialization = _build_model(config)
    model.fc = torch.nn.Identity()
    model = model.to(device).eval()
    arrays = {}
    started = time.perf_counter()
    for partition in ("D", "V"):
        base = _make_base(config, partition)
        dataset = DownstreamDataset(base, None, int(config["run"]["seed"]), training=False)
        loader = DataLoader(
            dataset, batch_size=256, shuffle=False, num_workers=int(config["training"]["num_workers"]),
            pin_memory=device.type == "cuda",
        )
        features, ids = [], []
        with torch.inference_mode():
            for images, _, patch_ids in loader:
                features.append(model(images.to(device, non_blocking=True)).float().cpu().numpy())
                ids.extend(map(str, patch_ids))
        arrays[f"{partition}_features"] = np.concatenate(features).astype(np.float16)
        arrays[f"{partition}_patch_ids"] = np.asarray(ids, dtype="U")
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("wb") as stream:
        np.savez_compressed(stream, **arrays)
    result = {
        "model_id": config["run"]["model_id"], "seed": config["run"]["seed"],
        "initialization": initialization, "feature_dimension": 512,
        "d_patches": len(arrays["D_patch_ids"]), "v_patches": len(arrays["V_patch_ids"]),
        "path": str(output_path), "sha256": file_sha256(output_path),
        "elapsed_seconds": time.perf_counter() - started,
        "evaluation_labels_loaded": False,
    }
    output_path.with_suffix(".json").write_text(json.dumps(result, indent=2) + "\n")
    return result
